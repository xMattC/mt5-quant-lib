//+------------------------------------------------------------------+
//| Multi-Symbol Strategy EA using MyLibs Framework                  |
//+------------------------------------------------------------------+
// Description:
// - This Expert Advisor (EA) implements a modular, multi-symbol trading 
//   strategy using the MyLibs library for MetaTrader 5.
// - The strategy combines RSI triggers, trendline analysis, ATR bands, 
//   confirmation and volume indicators, and manages both TP and runner 
//   trades with risk control.
//
// Features:
// - Modular logic for entry/exit using signal structs
// - Trailing stop and breakeven logic for runners
// - Custom fitness criteria for optimization (via OnTester())
// - Support for training/testing data splits
// - Time filtering for bar-based updates
//
// Usage:
// - Place the 'MyLibs' folder in MQL5/Include/
// - Compile this EA and attach it to any chart
// - Strategy supports multiple symbols simply modify the SymbolsArray
// - Adjust inputs for lot size, SL/TP modes, ATR periods, etc. as needed
//
// Notes:
// - All indicator handles are registered and released via ResourceManager
// - Trade logic only executes once per bar per symbol
// - Signals are tracked and time-decayed using MultiSymbolSignalTracker
//+------------------------------------------------------------------+
...
trading_signals build_trading_signals(bool is_long, string symbol, int tl_hand, int trig_hand, int conf_hand, int vol_hand, int exit_hand) {
    ...
    tl_tools.detect_cross(symbol, tl_hand, long_sigs.tl_cross, short_sigs.tl_cross, 1);
    tl_tools.detect_cross(symbol, tl_hand, long_sigs.tl_cross_prev, short_sigs.tl_cross_prev, 2);
    tl_tools.trend_direction(symbol, tl_hand, long_sigs.tl_agrees, short_sigs.tl_agrees, 1);
    tl_tools.trend_direction(symbol, tl_hand, long_sigs.tl_agrees_prev, short_sigs.tl_agrees_prev, 2);
    ...
    long_sigs.atr_b = atr_bands.inside_upper_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 1);
    short_sigs.atr_b = atr_bands.inside_lower_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 1);
    long_sigs.atr_b_prev = atr_bands.inside_upper_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 2);
    short_sigs.atr_b_prev = atr_bands.inside_lower_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 2);
    long_sigs.atr_pullback = atr_bands.crossed_below_upper_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 1);
    short_sigs.atr_pullback = atr_bands.crossed_above_lower_band(symbol, tl_hand, 14, PERIOD_CURRENT, 1.0, 1);
...
bool standard_entry(const trading_signals& data) {
    ...
    bool delayed_entry_valid = trigger_sig_prv && tl_agree_prev && inside_atr_band_prev &&  // Previous candle
                               trigger_agrees && tl_agree && confirmation && volume;        // Current candle
...
bool trendline_cross_entry(const trading_signals& data) {
    ...
    bool delayed_entry_valid = 
        ...
        trigger_sig_recent && trigger_agrees && tl_agrees && confirmation && volume;  // Current candle
...
void manage_orders(string symbol, bool long_in, bool short_in, bool exit_long, bool exit_short) {
    ...
    if (entry_orders.count_open_positions(symbol, 0, magic_number) > 0) return;
    if (entry_orders.count_open_positions(symbol, 0, runner_magic) > 0) return;
...
void trigger(int handle, bool& long_sig, bool& short_sig, bool& long_agree, bool& short_agree, int shift = 1) {
    ...
    double curr = m_utils.get_buffer_value(handle, shift);
    double prev = m_utils.get_buffer_value(handle, shift + 1);
    if (curr == EMPTY_VALUE || prev == EMPTY_VALUE) return;

    long_sig = (prev < 50 && curr > 50);
    short_sig = (prev > 50 && curr < 50);
    long_agree = (curr > 50);
    short_agree = (curr < 50);
}
...
void conformation(int handle, bool& long_ok, bool& short_ok) {
    ...
    double value = m_utils.get_buffer_value(handle, 1);
...
void volume_filter(int handle, bool& long_ok, bool& short_ok) {
    ...
    double curr = m_utils.get_buffer_value(handle, 1);
    double prev = m_utils.get_buffer_value(handle, 2);
    if (curr == EMPTY_VALUE || prev == EMPTY_VALUE) return;
...
void exit_signal(int handle, bool& exit_long, bool& exit_short) {
    ...
    exit_long = (prev > 50 && curr < 50);
    exit_short = (prev < 50 && curr > 50);
}
